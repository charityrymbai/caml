...
const Flashcards = () => {
  ...
  return (
    <div className="bg-black pt-20">
      ...
      {screenWidth > 768 ? (
        <DesktopFlash data={data} />
      ) : (
        <MobileFlash data={data} />
      )}
    </div>
  );
};
            ...
            <span className="wrapper_flashcard">
              <span className="content_flashcard">
                <span className="face back">
                  <div className="back-top p-2 mt-10 font-bold text-xl text-[#4d0707] ">
                    {card.q}
                  </div>
                  <div>😅😉</div>
                </span>
                <span className="face front">
                  <div className="front-top p-2 mt-10 font-bold text-xl text-[#054a03]">
                    {card.a}
                  </div>
                  <div>😊😗</div>
                </span>
...
const MobileFlash = ({ data }) => {
  ...
  return (
    <div className="flex space-x-4 items-center justify-center min-h-screen bg-black text-white">
      ...
      >
        <div
          className={`absolute w-full h-full p-4 flex items-center justify-center backface-hidden ${showAnswer ? "hidden" : "block"}`}
        >
          <p className="text-lg text-red-900 font-bold drop-shadow-2xl">
            {data[currentCard].q}
          </p>
        </div>
        <div
          className={`absolute w-full h-full p-4 flex items-center justify-center backface-hidden transform rotate-y-180 ${showAnswer ? "block" : "hidden"}`}
        >
          <p className="text-lg text-green-900 font-bold drop-shadow-2xl">
            {data[currentCard].a}
          </p>
        </div>
      </div>
